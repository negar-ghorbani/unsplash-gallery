...
</div>`})};l.addEventListener("click",()=>{const n=document.getElementById("input"),t=`https://api.unsplash.com/search/photos/?client_id=${{}.VITE_CLIENT_ID}&query=${n.value}&fit=crop&h=200px&w=300px`,s=JSON.parse(localStorage.getItem("myList"));return n.value===""||s.includes(n.value)?(c(t),s):(s.push(n.value),localStorage.setItem("myList",JSON.stringify(s)),c(t),n.value="",!0)});const u=document.getElementById("input");u.addEventListener("focus",()=>{let n=JSON.parse(localStorage.getItem("myList"));const t=document.querySelector(".suggestions ul");if(t.innerHTML="",n.length>0){for(let s=0;s<n.length;s++){let e=n[s];e=e.replace(e,`<strong>${e}</strong>`),t.innerHTML+=`<li>${e}</li>`}t.classList.add("has-suggestions")}else n=[],t.innerHTML="",t.classList.remove("has-suggestions")});function a(n){const t=document.querySelector(".suggestions ul"),s=document.getElementById("input");s.value=n.target.innerText,s.focus(),t.innerHTML="",t.classList.remove("has-suggestions");const e=`https://api.unsplash.com/search/photos/?client_id=q4swtOlzLBR4BJxfQEgQA-u3_3hNzATacG7eJtjYLcU&query=${s.value}&fit=crop&h=200px&w=300px`;c(e)}const d=document.querySelector(".suggestions ul");d.addEventListener("click",a);
